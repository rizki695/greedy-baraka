...
st.title("🔮 Prediksi Spiral Greedy (2 Selalu Muncul)")
...
def predict_top_two(hist):
    ...
    if len(freq) < 2:
        global_freq = Counter(data["next_symbol"])
...
st.caption("🧠 Sistem by Baraka + GPT — versi final fix, 2 prediksi selalu tampil")
